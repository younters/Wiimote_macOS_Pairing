...
## Remote Bluetooth addresses

These are the user's three knockoff remotes, included at the user's request for cross-Mac diagnostics. They are remote device addresses, not the Mac's Bluetooth address. Do not compile them into a compatibility allowlist or count a test using these known addresses as successful automatic discovery.

| Remote | Bluetooth address | Advertised name | Observed Linux class | First-Mac result |
| --- | --- | --- | --- | --- |
| First clone | `00:17:AB:39:C7:CD` | `Nintendo RVL-CNT-01` | `0x002504` | Paired and received physical HID input; guided pairing also succeeded. |
| Second clone | `00:17:AB:39:9D:E9` | `Nintendo RVL-CNT-01` | `0x002504` | Paired and received physical HID input; HOME, Minus, and Plus verified. |
| Third clone | `00:17:AB:F2:C9:72` | `Nintendo RVL-CNT-01` | `0x042500` | ACL and HID service discovery reached; no application PIN callback or confirmed input. |

The user reports the third has the same brand and board as the second, with a different color. Physical color-to-address mapping has not been independently established. The official remote connected on the second Mac is a separate device whose address is not recorded here.

These addresses can support a controlled connection test to distinguish discovery failure from pairing failure. Pairing bonds remain per host; derive the red-SYNC PIN from the receiving Mac automatically. The end goal remains no address entry for new remotes.
